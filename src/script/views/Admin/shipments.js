...
// --- Add Tracking Update and Send Email to Receiver ---
async function addLiveTrackingUpdateAndNotify(shipmentId, location, status, note) {
  ...
  if (location.match(/^-?\d+\.\d+,-?\d+\.\d+$/)) {
    ...
    locationDisplay = location; // Show admin input only
  } else {
    locationDisplay = location; // Show admin input only
  }
  ...
  if (shipment?.email) {
    try {
      await sendEmail({ to: shipment.email, subject, html });
...
const shipments = async () => {
  ...
  async function getPresentLocation(shipmentId, destination) {
    const { data: updates } = await supabase
      .from("tracking_updates")
      .select("location")
      ...
      .limit(1);
    if (updates && updates.length > 0 && updates[0].location) {
      return updates[0].location;
    }
    return destination;
  }
  ...
  async function renderMain() {
    ...
    await Promise.all(
      shipmentsList.map(async (s) => {
        presentLocations[s.id] = await getPresentLocation(s.id, s.destination);
      })
    );
              ...
              <table class="min-w-full bg-white/10 rounded-xl border border-white/20 text-white shadow-lg">
                <thead>
                  <tr>
                    ...
                    <th class="px-4 py-2">Present Location</th>
                ...
                s.status === "in-transit" ? "bg-blue-500/20 text-blue-400" :
                    s.status === "processing" ? "bg-yellow-500/20 text-yellow-400" :
                      ...
                      <td class="px-4 py-2">${presentLocations[s.id]}</td>
            ...
            <div id="createShipmentModal" class="fixed inset-0 bg-black/60 hidden items-center justify-center z-50 flex">
              <div class="bg-white rounded-xl p-8 w-full max-w-md shadow-lg relative animate-slideIn overflow-auto" style="max-height:90vh;">
                ...
                <form id="createShipmentForm" class="space-y-4">
                  ...
                  <input type="email" name="email" placeholder="Receiver Email" class="input input-bordered w-full py-2 px-4 rounded-lg" required />
      ...
      <div id="editShipmentModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl p-8 w-full max-w-md shadow-lg relative animate-slideIn overflow-auto" style="max-height:90vh;">
          ...
          <form id="editShipmentForm" class="space-y-4">
            ...
            <input type="email" name="email" value="${shipment.email || ""}" placeholder="Receiver Email" class="input input-bordered w-full py-2 px-4 rounded-lg" required />
      ...
      <div id="liveTrackingModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl p-8 w-full max-w-lg shadow-lg relative animate-slideIn overflow-auto" style="max-height:90vh;">
          ...
          <form id="liveTrackingForm" class="space-y-4">
            <input type="text" name="location" placeholder="Current Location (city, country or lat,lon)" class="input input-bordered w-full py-2 px-4 rounded-lg" required />
          ...
          <main class="p-4 md:p-8 max-w-5xl mx-auto animate-fadeIn">
            ...
            <div class="flex flex-col md:flex-row gap-8 mb-8">
              <div class="flex-shrink-0 flex flex-col items-center">
                <img src="${shipment.image_url || packageIcon}" class="w-32 h-32 rounded-full border-4 border-blue-500 bg-white mb-4" alt="Package" />
                <div class="bg-white/10 rounded-lg p-4 w-full">
                  <h2 class="text-xl font-bold text-white mb-2">Receiver Info</h2>
                  <p class="text-blue-300"><b>Name:</b> ${shipment.receiver_name}</p>
                  <p class="text-blue-300"><b>Address:</b> ${shipment.receiver_address}</p>
                  <p class="text-blue-300"><b>Phone:</b> ${shipment.receiver_phone}</p>
                  <p class="text-blue-300"><b>Email:</b> ${shipment.email}</p>
                </div>
              </div>
              <div class="flex-1">
                <div class="bg-white/10 rounded-lg p-4 mb-4">
                  <h2 class="text-xl font-bold text-white mb-2">Sender Info</h2>
                  <p class="text-blue-300"><b>User ID:</b> ${shipment.sender_id}</p>
                  <p class="text-blue-300"><b>Origin:</b> ${shipment.origin}</p>
                </div>
                <div class="bg-white/10 rounded-lg p-4 mb-4">
                  <h2 class="text-xl font-bold text-white mb-2">Package Details</h2>
                  <p class="text-blue-300"><b>Description:</b> ${shipment.package_description}</p>
                  <p class="text-blue-300"><b>Weight:</b> ${shipment.weight || "N/A"} kg</p>
                  <p class="text-blue-300"><b>Cost:</b> $${shipment.cost}</p>
                  <p class="text-blue-300"><b>Status:</b> ${shipment.status}</p>
                  <p class="text-blue-300"><b>Tracking:</b> ${shipment.tracking_code}</p>
  ...
  function mainEvents() {
    ...
    document.querySelectorAll("#shipmentsTableBody tr").forEach(row => {
      ...
      row.querySelector(".approveShipmentBtn")?.addEventListener("click", async (e) => {
        e.stopPropagation();
        await addLiveTrackingUpdateAndNotify(shipmentId, encodeLatLon(...WAREHOUSE_LOCATION), "approved", "Approved by admin");
        shipmentsList = await fetchShipments();
        renderMain();
      });
